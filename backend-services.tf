...
resource "aws_db_instance" "vprofile_rds" {
  ...
  username                = var.dbuser
  password                = var.dbpass
  parameter_group_name    = "default.mysql5.6" # Verify this name in your AWS console
  multi_az                = false
  publicly_accessible     = false
  skip_final_snapshot     = true
  db_subnet_group_name    = aws_db_subnet_group.vprofile-rds-subgrp.name
  vpc_security_group_ids  = [aws_security_group.vprofile-backend-sg.id] # Replace with the correct security group reference
}
  ...
  subnet_group_name      = aws_elasticache_subnet_group.vprofile-ecache-subgrp.id
